...
public class ResourceChecker : EditorWindow {
	...
	string[] inspectToolbarStrings = {"贴图", "材质球","网格"};
	string[] inspectToolbarStrings2 = {"贴图", "材质球","网格", "缺失"};
	...
	void OnGUI ()
	{
		...
		IncludeSpriteAnimations = GUILayout.Toggle(IncludeSpriteAnimations, "查看 sprite 动画", GUILayout.Width(300));
		...
		IncludeGuiElements = GUILayout.Toggle(IncludeGuiElements, "查看GUI元素", GUILayout.Width(300));
		IncludeLightmapTextures = GUILayout.Toggle(IncludeLightmapTextures, "查看光照贴图纹理", GUILayout.Width(300));
		GUI.color = defColor;
		GUILayout.BeginArea(new Rect(position.width-85,5,100,65));
		if (GUILayout.Button("计算",GUILayout.Width(80), GUILayout.Height(40)))
			CheckResources();
		...
		if (thingsMissing == true) {
			EditorGUI.HelpBox (new Rect(8,93,300,25),"一些游戏对象缺失图形元素", MessageType.Error);
		}
		GUILayout.BeginHorizontal();
		GUILayout.Label("贴图 "+ActiveTextures.Count+" - "+FormatSizeString(TotalTextureMemory));
		GUILayout.Label("材质球 "+ActiveMaterials.Count);
		GUILayout.Label("网格 "+ActiveMeshDetails.Count+" - "+TotalMeshVertices+" 个顶点");
	...
	void ListTextures()
	{
		...
		if (ActiveTextures.Count>0)
		{
			...
			if(GUILayout.Button("选择\n所有",GUILayout.Width(ThumbnailWidth*2)))
			{
